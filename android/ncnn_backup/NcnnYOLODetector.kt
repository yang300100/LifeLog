...
class NcnnYOLODetector(
    ...
    private var ncnnAvailable: Boolean = false

    override suspend fun load() {
        if (isReady) return
        withContext(Dispatchers.IO) {
            try {
                // 复制模型文件到内部存储 (NCNN 从文件路径加载)
                ...
                // 加载网络
                netPtr = nativeLoadModel(paramFile.absolutePath, binFile.absolutePath)
                if (netPtr == 0L) throw Exception("NCNN 模型加载失败")

                isReady = true
                Log.i(TAG, "NCNN YOLOv8n 加载成功")
            } catch (e: Exception) {
                Log.e(TAG, "NCNN 加载失败: ${e.message}. 将自动回退到 HeuristicSegmenter")
                ncnnAvailable = false
            }
        }
    }
    ...
    override suspend fun segment(sceneBitmap: Bitmap): SegmentResult = withContext(Dispatchers.Default) {
        ...
        // 后处理: 映射到原图坐标
        val objects = detections.mapIndexed { i, det ->
            val x1 = (det.x1 * w).toInt().coerceIn(0, w)
            val y1 = (det.y1 * h).toInt().coerceIn(0, h)
            val x2 = (det.x2 * w).toInt().coerceIn(0, w)
            val y2 = (det.y2 * h).toInt().coerceIn(0, h)
            val cx = (x1 + x2) / 2
            val cy = (y1 + y2) / 2
            val area = (x2 - x1).toFloat() * (y2 - y1).toFloat()
            val className = if (det.classId in COCO_NAMES.indices) COCO_NAMES[det.classId] else "unknown"
            val depth = when {
        ...
        Log.i(TAG, "YOLO 检测: ${objects.size} 个物体, ${elapsed}ms")
